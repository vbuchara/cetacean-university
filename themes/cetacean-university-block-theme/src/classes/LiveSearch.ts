...
export default class LiveSearch {
    public openSearchTrigger: JQuery<HTMLElement>;
    public closeSearchTrigger: JQuery<HTMLElement>;
    public inputSearch: JQuery<HTMLElement>;
    public searchOverlay: JQuery<HTMLElement>;
    public searchResults: JQuery<HTMLElement>;
    public isSearchOverlayOpen: boolean;
    public isShowingLoadingSpinner: boolean;
    ...
    openSearchOverlay(event?: JQuery.Event) {
    ...
    onOpenSearchKeyReleased(event: JQuery.Event){
        if(
            event.key !== "s" || 
    ...
    onCloseSearchKeyReleased(event: JQuery.Event) {
        if(
            event.key !== "Escape" 
            || !this.isSearchOverlayOpen
        ) return;
    ...
    onSearchInput(event: JQuery.UIEventBase<HTMLInputElement, HTMLInputElement, any, HTMLInputElement>){        
    ...
    async getSearchResults(event: JQuery.UIEventBase<HTMLInputElement, HTMLInputElement, any, HTMLInputElement>){
        try {
            const { root_url } = cetaceanUniversityData;

            const allPosts: SearchGet = await $.getJSON(
